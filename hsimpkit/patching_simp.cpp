...
bool PatchingSimp::mergeSimpPly(uint target_vert) {
	...
	for (i = 0; i < patchIndices.count(); i ++) {
		if (!patch.pairCollapse(tmp_base, patchIndices[i], simp_verts, vert_count, 
			 target_vert, vert_stream, face_stream, bound_id_map, verts_gen)) {
			cerr << "#ERROR: simplifying patch " << patchIndices[i].i << "_" << patchIndices[i].j 
	...
	out_name += getFilename(file_name) + "_psimp.ply";
	...
	for (i = 0; i < face_stream.count(); i ++) {
		...
		if (!fout.good()) {
			cerr << "#ERROR: writing face for the simplified mesh failed" << endl;
			return false;
		}
	}

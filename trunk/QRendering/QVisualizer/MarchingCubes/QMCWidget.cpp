 ...
 * 
 * @file    QMCWidget.cpp
 * @brief   QMCWidget class declaration.
 * 
 * This file declares ...
 * 
 * @version 1.0
...
unsigned char QMCWidget::destroy()
{
    if (this->clQueue)
    {
...
unsigned char QMCWidget::initConnections(QMCControlPanel* panel)
{
    this->panel = panel;

    const Ui::QMCControlPanel* ui = panel->getUI();
...
// Step 2 - init data
unsigned char QMCWidget::initData(const std::string &name)
{
    dataFileName = name;
...
unsigned char QMCWidget::initOpenCL()
{
...
unsigned char QMCWidget::initConfigurations()
{
	// init cache volume data
	...
	std::cout << " > INFO: volume data size: " << volumeSize.s[0] << " x " << volumeSize.s[1] << " x " << volumeSize.s[2] << std::endl;
...
unsigned char QMCWidget::initPrograms()
{
    cl_int status = CL_SUCCESS;
...
unsigned char QMCWidget::initArguments()
{
    cl_int status = CL_SUCCESS;
...
unsigned char QMCWidget::slotUpdateIsoValue()
{
...
// resizeGL
void QMCWidget::resizeGL(int w, int h)
{
    if (error) return;
    ...
    glLoadIdentity();
    gluPerspective(45.0f, (GLfloat)settings.width / settings.height, 1.5f, 10000.0f);

	updateGL();
}

// paintGL
void QMCWidget::paintGL()
{
    if (error) return;
	...
	if (isoValue != settings.isoValue)
	{
		...
		if (totalNumber == 0)
		{
			...
			std::cout << " > INFO: iso value = " << isoValue << ", triangles amount = " << totalNumber << "." << std::endl;
		}
...
unsigned char QMCWidget::classifyCubes()
{
	cl_int status = CL_SUCCESS;
...
unsigned char QMCWidget::constructHP()
{
    cl_int status = CL_SUCCESS;
...
unsigned char QMCWidget::traverseHP(cl_uint size)
{
	cl_int status = CL_SUCCESS;
...
void QMCWidget::mousePressEvent(QMouseEvent *event)
{
    mouseX = event->x();
    mouseY = event->y();
...
void QMCWidget::mouseMoveEvent(QMouseEvent *event)
{
...
void QMCWidget::wheelEvent(QWheelEvent *event)
{
    updateGL();
}

// keyPressEvent
void QMCWidget::keyPressEvent(QKeyEvent *event)
{
    switch (event->key())
    {
    case Qt::Key_Escape:
...
void QMCWidget::saveSettings()
{}

void QMCWidget::loadSettings()
{}

void QMCWidget::printSettings()
{

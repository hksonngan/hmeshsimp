...
char *input_ply_filename = NULL;
char input_smf_filename[500];
...
void output_final_model()
{
    ...
    case VRML:
	cleanup_for_output();
	trimExtAndAppend(input_smf_filename, buf, "_qslim.vrml");
	fout.open(buf);
	output_vrml(fout);
	break;
	...
	case PLY:
	cleanup_for_output();
	trimExtAndAppend(input_smf_filename, buf, "_qslim.ply");
	output_ply(m, buf);
	break;
    }
...
{
    if( streq(filename, "-") ) // str equal, read files from standard input - ht
		smf->read(cin, m);
    else
    {

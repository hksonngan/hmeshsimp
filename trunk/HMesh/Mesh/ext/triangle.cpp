...
/*   an edge, on a vertex, or outside the mesh.                              */
		...
		typedef TRIANGLE_REAL **triangle;            /* Really:  typedef triangle *triangle   */
		...
		typedef TRIANGLE_REAL **subseg;                  /* Really:  typedef subseg *subseg   */
		...
		struct badtriang {
			...
			TRIANGLE_REAL key;                             /* cos^2 of smallest (apical) angle. */
		...
		struct event {
			TRIANGLE_REAL xkey, ykey;                              /* Coordinates of the event. */
		...
		TRIANGLE_REAL splitter;       /* Used to split TRIANGLE_REAL factors for exact multiplication. */
		TRIANGLE_REAL epsilon;                             /* Floating-point machine epsilon. */
		TRIANGLE_REAL resulterrbound;
		TRIANGLE_REAL ccwerrboundA, ccwerrboundB, ccwerrboundC;
		TRIANGLE_REAL iccerrboundA, iccerrboundB, iccerrboundC;
		TRIANGLE_REAL o3derrboundA, o3derrboundB, o3derrboundC;
		...
		struct mesh {
			...
			TRIANGLE_REAL xmin, xmax, ymin, ymax;                            /* x and y bounds. */
			TRIANGLE_REAL xminextreme;      /* Nonexistent x value used as a flag in sweepline. */
		...
		struct behavior {
			...
			int nobisect;
			int steiner;
			TRIANGLE_REAL minangle, goodangle, offconstant;
			TRIANGLE_REAL maxarea;
...
#define elemattribute(otri, attnum)                                           \
	((TRIANGLE_REAL *) (otri).tri)[m->elemattribindex + (attnum)]

#define setelemattribute(otri, attnum, value)                                 \
	((TRIANGLE_REAL *) (otri).tri)[m->elemattribindex + (attnum)] = value

		/* Check or set a triangle's maximum area bound.                             */

#define areabound(otri)  ((TRIANGLE_REAL *) (otri).tri)[m->areaboundindex]

#define setareabound(otri, value)                                             \
	((TRIANGLE_REAL *) (otri).tri)[m->areaboundindex] = value
...
#define ANSI_DECLARATORS
		...
		int triunsuitable(vertex triorg, vertex tridest, vertex triapex, TRIANGLE_REAL area)
		...
		TRIANGLE_REAL area;                                      /* The area of the triangle. */
#endif /* not ANSI_DECLARATORS */

		{
			TRIANGLE_REAL dxoa, dxda, dxod;
			TRIANGLE_REAL dyoa, dyda, dyod;
			TRIANGLE_REAL oalen, dalen, odlen;
			TRIANGLE_REAL maxlen;
		...
		void info()
		{
			...
			//printf(
				"        a posteriori error tri_estimates.  A .poly file can optionally contain\n"
				);
			//printf(
			...
			//printf(
				"  sequence of meshes using error tri_estimate-driven mesh refinement.\n");
			//printf("\n");
			//printf(
			...
			//printf(
				"  are typically based on a posteriori error tri_estimates resulting from a\n");
			...
			//printf("\n");
			//printf(
				"  The -V switch produces extended statistics, including a rough tri_estimate\n");
			//printf(
						...
						if (argv[i][j] == 'q') {
							...
							if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
								(argv[i][j + 1] == '.')) {
									...
									workstring[k] = '\0';
									b->minangle = (TRIANGLE_REAL) strtod(workstring, (char **) NULL);
							} else {
								b->minangle = 20.0;
							}
						}
						if (argv[i][j] == 'a') {
							...
							if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
								(argv[i][j + 1] == '.')) {
									...
									workstring[k] = '\0';
									b->maxarea = (TRIANGLE_REAL) strtod(workstring, (char **) NULL);
									if (b->maxarea <= 0.0) {
...
//					printtri.orient);
...

		{
			...
			m->vertexmarkindex = ((m->mesh_dim + m->nextras) * sizeof(TRIANGLE_REAL) +
				sizeof(int) - 1) /
				sizeof(int);
...

		{
			...
			m->elemattribindex = (trisize + sizeof(TRIANGLE_REAL) - 1) / sizeof(TRIANGLE_REAL);
			...
			if (b->vararea) {
				trisize = (m->areaboundindex + 1) * sizeof(TRIANGLE_REAL);
			} else if (m->eextras + b->regionattrib > 0) {
				trisize = m->areaboundindex * sizeof(TRIANGLE_REAL);
			}
...
#define Split(a, ahi, alo) \
	c = (TRIANGLE_REAL) (splitter * a); \
	abig = (TRIANGLE_REAL) (c - a); \
	ahi = c - abig; \
	alo = a - ahi
...
#define Two_One_Product(a1, a0, b, x3, x2, x1, x0) \
	...
	Fast_Two_Sum(_j, _k, x3, x2)
		...
		/*****************************************************************************/
		/*                                                                           */
		/*  tri_fast_expansion_sum_zeroelim()   Sum two expansions, eliminating zero     */
...
#ifdef ANSI_DECLARATORS
		int tri_fast_expansion_sum_zeroelim(int elen, TRIANGLE_REAL *e, int flen, TRIANGLE_REAL *f, TRIANGLE_REAL *h)
#else /* not ANSI_DECLARATORS */
		int tri_fast_expansion_sum_zeroelim(elen, e, flen, f, h)  /* h cannot be e or f. */
			int elen;
		TRIANGLE_REAL *e;
		int flen;
		TRIANGLE_REAL *f;
		...
		{
			TRIANGLE_REAL Q;
			INEXACT TRIANGLE_REAL Qnew;
			INEXACT TRIANGLE_REAL hh;
			INEXACT TRIANGLE_REAL bvirt;
			TRIANGLE_REAL avirt, bround, around;
			int eindex, findex, hindex;
			TRIANGLE_REAL enow, fnow;
		...
		/*****************************************************************************/
		/*                                                                           */
		/*  tri_scale_expansion_zeroelim()   Multiply an expansion by a scalar,          */
...
#ifdef ANSI_DECLARATORS
		int tri_scale_expansion_zeroelim(int elen, TRIANGLE_REAL *e, TRIANGLE_REAL b, TRIANGLE_REAL *h)
#else /* not ANSI_DECLARATORS */
		int tri_scale_expansion_zeroelim(elen, e, b, h)   /* e and h cannot be the same. */
			int elen;
		TRIANGLE_REAL *e;
		TRIANGLE_REAL b;
		TRIANGLE_REAL *h;
		...
		{
			INEXACT TRIANGLE_REAL Q, sum;
			TRIANGLE_REAL hh;
			INEXACT TRIANGLE_REAL product1;
			TRIANGLE_REAL product0;
			int eindex, hindex;
			TRIANGLE_REAL enow;
			INEXACT TRIANGLE_REAL bvirt;
			TRIANGLE_REAL avirt, bround, around;
			INEXACT TRIANGLE_REAL c;
			INEXACT TRIANGLE_REAL abig;
			TRIANGLE_REAL ahi, alo, bhi, blo;
			TRIANGLE_REAL err1, err2, err3;
		...
		/*****************************************************************************/
		/*                                                                           */
		/*  tri_estimate()   Produce a one-word estimate of an expansion's value.        */
		/*                                                                           */
...
#ifdef ANSI_DECLARATORS
		TRIANGLE_REAL tri_estimate(int elen, TRIANGLE_REAL *e)
#else /* not ANSI_DECLARATORS */
		TRIANGLE_REAL tri_estimate(elen, e)
			int elen;
		TRIANGLE_REAL *e;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
		TRIANGLE_REAL counterclockwiseadapt(vertex pa, vertex pb, vertex pc, TRIANGLE_REAL detsum)
#else /* not ANSI_DECLARATORS */
		TRIANGLE_REAL counterclockwiseadapt(pa, pb, pc, detsum)
			vertex pa;
		vertex pb;
		vertex pc;
		TRIANGLE_REAL detsum;
#endif /* not ANSI_DECLARATORS */

		{
			INEXACT TRIANGLE_REAL acx, acy, bcx, bcy;
			TRIANGLE_REAL acxtail, acytail, bcxtail, bcytail;
			INEXACT TRIANGLE_REAL detleft, detright;
			TRIANGLE_REAL detlefttail, detrighttail;
			TRIANGLE_REAL det, errbound;
			TRIANGLE_REAL B[4], C1[8], C2[12], D[16];
			INEXACT TRIANGLE_REAL B3;
			int C1length, C2length, Dlength;
			TRIANGLE_REAL u[4];
			INEXACT TRIANGLE_REAL u3;
			INEXACT TRIANGLE_REAL s1, t1;
			TRIANGLE_REAL s0, t0;
			...
			TRIANGLE_REAL ahi, alo, bhi, blo;
			TRIANGLE_REAL err1, err2, err3;
			INEXACT TRIANGLE_REAL _i, _j;
			TRIANGLE_REAL _0;

			acx = (TRIANGLE_REAL) (pa[0] - pc[0]);
			bcx = (TRIANGLE_REAL) (pb[0] - pc[0]);
			acy = (TRIANGLE_REAL) (pa[1] - pc[1]);
			bcy = (TRIANGLE_REAL) (pb[1] - pc[1]);
...
#ifdef ANSI_DECLARATORS
		TRIANGLE_REAL counterclockwise(struct mesh *m, struct behavior *b,
			vertex pa, vertex pb, vertex pc)
#else /* not ANSI_DECLARATORS */
		TRIANGLE_REAL counterclockwise(m, b, pa, pb, pc)
		struct mesh *m;
		struct behavior *b;
		vertex pa;
		vertex pb;
		...
		{
			TRIANGLE_REAL detleft, detright, det;
			TRIANGLE_REAL detsum, errbound;
...
#ifdef ANSI_DECLARATORS
		TRIANGLE_REAL tri_incircleadapt(vertex pa, vertex pb, vertex pc, vertex pd, TRIANGLE_REAL permanent)
#else /* not ANSI_DECLARATORS */
		TRIANGLE_REAL tri_incircleadapt(pa, pb, pc, pd, permanent)
			vertex pa;
		vertex pb;
		vertex pc;
		vertex pd;
		TRIANGLE_REAL permanent;
#endif /* not ANSI_DECLARATORS */

		{
			INEXACT TRIANGLE_REAL adx, bdx, cdx, ady, bdy, cdy;
			TRIANGLE_REAL det, errbound;

			INEXACT TRIANGLE_REAL bdxcdy1, cdxbdy1, cdxady1, adxcdy1, adxbdy1, bdxady1;
			TRIANGLE_REAL bdxcdy0, cdxbdy0, cdxady0, adxcdy0, adxbdy0, bdxady0;
			TRIANGLE_REAL bc[4], ca[4], ab[4];
			INEXACT TRIANGLE_REAL bc3, ca3, ab3;
			TRIANGLE_REAL axbc[8], axxbc[16], aybc[8], ayybc[16], adet[32];
			int axbclen, axxbclen, aybclen, ayybclen, alen;
			TRIANGLE_REAL bxca[8], bxxca[16], byca[8], byyca[16], bdet[32];
			int bxcalen, bxxcalen, bycalen, byycalen, blen;
			TRIANGLE_REAL cxab[8], cxxab[16], cyab[8], cyyab[16], cdet[32];
			int cxablen, cxxablen, cyablen, cyyablen, clen;
			TRIANGLE_REAL abdet[64];
			int ablen;
			TRIANGLE_REAL fin1[1152], fin2[1152];
			TRIANGLE_REAL *finnow, *finother, *finswap;
			int finlength;

			TRIANGLE_REAL adxtail, bdxtail, cdxtail, adytail, bdytail, cdytail;
			INEXACT TRIANGLE_REAL adxadx1, adyady1, bdxbdx1, bdybdy1, cdxcdx1, cdycdy1;
			TRIANGLE_REAL adxadx0, adyady0, bdxbdx0, bdybdy0, cdxcdx0, cdycdy0;
			TRIANGLE_REAL aa[4], bb[4], cc[4];
			INEXACT TRIANGLE_REAL aa3, bb3, cc3;
			INEXACT TRIANGLE_REAL ti1, tj1;
			TRIANGLE_REAL ti0, tj0;
			TRIANGLE_REAL u[4], v[4];
			INEXACT TRIANGLE_REAL u3, v3;
			TRIANGLE_REAL temp8[8], temp16a[16], temp16b[16], temp16c[16];
			TRIANGLE_REAL temp32a[32], temp32b[32], temp48[48], temp64[64];
			int temp8len, temp16alen, temp16blen, temp16clen;
			int temp32alen, temp32blen, temp48len, temp64len;
			TRIANGLE_REAL axtbb[8], axtcc[8], aytbb[8], aytcc[8];
			int axtbblen, axtcclen, aytbblen, aytcclen;
			TRIANGLE_REAL bxtaa[8], bxtcc[8], bytaa[8], bytcc[8];
			int bxtaalen, bxtcclen, bytaalen, bytcclen;
			TRIANGLE_REAL cxtaa[8], cxtbb[8], cytaa[8], cytbb[8];
			int cxtaalen, cxtbblen, cytaalen, cytbblen;
			TRIANGLE_REAL axtbc[8], aytbc[8], bxtca[8], bytca[8], cxtab[8], cytab[8];
			int axtbclen, aytbclen, bxtcalen, bytcalen, cxtablen, cytablen;
			TRIANGLE_REAL axtbct[16], aytbct[16], bxtcat[16], bytcat[16], cxtabt[16], cytabt[16];
			int axtbctlen, aytbctlen, bxtcatlen, bytcatlen, cxtabtlen, cytabtlen;
			TRIANGLE_REAL axtbctt[8], aytbctt[8], bxtcatt[8];
			TRIANGLE_REAL bytcatt[8], cxtabtt[8], cytabtt[8];
			int axtbcttlen, aytbcttlen, bxtcattlen, bytcattlen, cxtabttlen, cytabttlen;
			TRIANGLE_REAL abt[8], bct[8], cat[8];
			int abtlen, bctlen, catlen;
			TRIANGLE_REAL abtt[4], bctt[4], catt[4];
			int abttlen, bcttlen, cattlen;
			INEXACT TRIANGLE_REAL abtt3, bctt3, catt3;
			TRIANGLE_REAL negate;
			...
			adx = (TRIANGLE_REAL) (pa[0] - pd[0]);
			bdx = (TRIANGLE_REAL) (pb[0] - pd[0]);
			cdx = (TRIANGLE_REAL) (pc[0] - pd[0]);
			ady = (TRIANGLE_REAL) (pa[1] - pd[1]);
			bdy = (TRIANGLE_REAL) (pb[1] - pd[1]);
			cdy = (TRIANGLE_REAL) (pc[1] - pd[1]);
...
#ifdef ANSI_DECLARATORS
		TRIANGLE_REAL incircle(struct mesh *m, struct behavior *b,
			vertex pa, vertex pb, vertex pc, vertex pd)
#else /* not ANSI_DECLARATORS */
		TRIANGLE_REAL incircle(m, b, pa, pb, pc, pd)
		struct mesh *m;
		struct behavior *b;
		vertex pa;
		vertex pb;
		...
		{
			TRIANGLE_REAL adx, bdx, cdx, ady, bdy, cdy;
			TRIANGLE_REAL bdxcdy, cdxbdy, cdxady, adxcdy, adxbdy, bdxady;
			TRIANGLE_REAL alift, blift, clift;
			TRIANGLE_REAL det;
			TRIANGLE_REAL permanent, errbound;
...
#ifdef ANSI_DECLARATORS
		TRIANGLE_REAL orient3dadapt(vertex pa, vertex pb, vertex pc, vertex pd,
			TRIANGLE_REAL aheight, TRIANGLE_REAL bheight, TRIANGLE_REAL cheight, TRIANGLE_REAL dheight,
			TRIANGLE_REAL permanent)
#else /* not ANSI_DECLARATORS */
		TRIANGLE_REAL orient3dadapt(pa, pb, pc, pd,
			aheight, bheight, cheight, dheight, permanent)
			vertex pa;
		vertex pb;
		vertex pc;
		vertex pd;
		TRIANGLE_REAL aheight;
		TRIANGLE_REAL bheight;
		TRIANGLE_REAL cheight;
		TRIANGLE_REAL dheight;
		TRIANGLE_REAL permanent;
#endif /* not ANSI_DECLARATORS */

		{
			INEXACT TRIANGLE_REAL adx, bdx, cdx, ady, bdy, cdy, adheight, bdheight, cdheight;
			TRIANGLE_REAL det, errbound;
			...
			TRIANGLE_REAL bc[4], ca[4], ab[4];
			INEXACT TRIANGLE_REAL bc3, ca3, ab3;
			TRIANGLE_REAL adet[8], bdet[8], cdet[8];
			int alen, blen, clen;
			TRIANGLE_REAL abdet[16];
			int ablen;
			TRIANGLE_REAL *finnow, *finother, *finswap;
			TRIANGLE_REAL fin1[192], fin2[192];
			int finlength;

			TRIANGLE_REAL adxtail, bdxtail, cdxtail;
			TRIANGLE_REAL adytail, bdytail, cdytail;
			TRIANGLE_REAL adheighttail, bdheighttail, cdheighttail;
			INEXACT TRIANGLE_REAL at_blarge, at_clarge;
			INEXACT TRIANGLE_REAL bt_clarge, bt_alarge;
			INEXACT TRIANGLE_REAL ct_alarge, ct_blarge;
			TRIANGLE_REAL at_b[4], at_c[4], bt_c[4], bt_a[4], ct_a[4], ct_b[4];
			int at_blen, at_clen, bt_clen, bt_alen, ct_alen, ct_blen;
			INEXACT TRIANGLE_REAL bdxt_cdy1, cdxt_bdy1, cdxt_ady1;
			INEXACT TRIANGLE_REAL adxt_cdy1, adxt_bdy1, bdxt_ady1;
			TRIANGLE_REAL bdxt_cdy0, cdxt_bdy0, cdxt_ady0;
			TRIANGLE_REAL adxt_cdy0, adxt_bdy0, bdxt_ady0;
			INEXACT TRIANGLE_REAL bdyt_cdx1, cdyt_bdx1, cdyt_adx1;
			INEXACT TRIANGLE_REAL adyt_cdx1, adyt_bdx1, bdyt_adx1;
			TRIANGLE_REAL bdyt_cdx0, cdyt_bdx0, cdyt_adx0;
			TRIANGLE_REAL adyt_cdx0, adyt_bdx0, bdyt_adx0;
			TRIANGLE_REAL bct[8], cat[8], abt[8];
			int bctlen, catlen, abtlen;
			INEXACT TRIANGLE_REAL bdxt_cdyt1, cdxt_bdyt1, cdxt_adyt1;
			INEXACT TRIANGLE_REAL adxt_cdyt1, adxt_bdyt1, bdxt_adyt1;
			TRIANGLE_REAL bdxt_cdyt0, cdxt_bdyt0, cdxt_adyt0;
			TRIANGLE_REAL adxt_cdyt0, adxt_bdyt0, bdxt_adyt0;
			TRIANGLE_REAL u[4], v[12], w[16];
			INEXACT TRIANGLE_REAL u3;
			int vlength, wlength;
			TRIANGLE_REAL negate;
			...
			TRIANGLE_REAL ahi, alo, bhi, blo;
			TRIANGLE_REAL err1, err2, err3;
			INEXACT TRIANGLE_REAL _i, _j, _k;
			TRIANGLE_REAL _0;
			...
			bdy = (TRIANGLE_REAL) (pb[1] - pd[1]);
			cdy = (TRIANGLE_REAL) (pc[1] - pd[1]);
			adheight = (TRIANGLE_REAL) (aheight - dheight);
			bdheight = (TRIANGLE_REAL) (bheight - dheight);
			cdheight = (TRIANGLE_REAL) (cheight - dheight);
...
#ifdef ANSI_DECLARATORS
		TRIANGLE_REAL orient3d(struct mesh *m, struct behavior *b,
			vertex pa, vertex pb, vertex pc, vertex pd,
			TRIANGLE_REAL aheight, TRIANGLE_REAL bheight, TRIANGLE_REAL cheight, TRIANGLE_REAL dheight)
#else /* not ANSI_DECLARATORS */
		TRIANGLE_REAL orient3d(m, b, pa, pb, pc, pd, aheight, bheight, cheight, dheight)
		struct mesh *m;
		struct behavior *b;
		vertex pa;
		vertex pb;
		...
		{
			TRIANGLE_REAL adx, bdx, cdx, ady, bdy, cdy, adheight, bdheight, cdheight;
			TRIANGLE_REAL bdxcdy, cdxbdy, cdxady, adxcdy, adxbdy, bdxady;
			TRIANGLE_REAL det;
			TRIANGLE_REAL permanent, errbound;
...
#ifdef ANSI_DECLARATORS
		TRIANGLE_REAL nonregular(struct mesh *m, struct behavior *b,
			vertex pa, vertex pb, vertex pc, vertex pd)
#else /* not ANSI_DECLARATORS */
		TRIANGLE_REAL nonregular(m, b, pa, pb, pc, pd)
		struct mesh *m;
		struct behavior *b;
		vertex pa;
		vertex pb;
		...
		void findcircumcenter(struct mesh *m, struct behavior *b,
			...
			vertex circumcenter, TRIANGLE_REAL *xi, TRIANGLE_REAL *eta, int offcenter)
#else /* not ANSI_DECLARATORS */
		...
		{
			TRIANGLE_REAL xdo, ydo, xao, yao;
			TRIANGLE_REAL dodist, aodist, dadist;
			TRIANGLE_REAL denominator;
			TRIANGLE_REAL dx, dy, dxoff, dyoff;
			...
			/*   serves as a conservative tri_estimate of the insertion radius of the */
			/*   circumcenter's parent.  The tri_estimate is used to ensure that      */
		...
		{
			TRIANGLE_REAL length, multiplier;
		...
		void enqueuebadtri(struct mesh *m, struct behavior *b, struct otri *enqtri,
			TRIANGLE_REAL minedge, vertex enqapex, vertex enqorg, vertex enqdest)
#else /* not ANSI_DECLARATORS */
		void enqueuebadtri(m, b, enqtri, minedge, enqapex, enqorg, enqdest)
		struct mesh *m;
		struct behavior *b;
		struct otri *enqtri;
		TRIANGLE_REAL minedge;
		vertex enqapex;
		vertex enqorg;
		vertex enqdest;
#endif /* not ANSI_DECLARATORS */
...

		{
			...
			vertex joinvertex;
			TRIANGLE_REAL dxod, dyod, dxda, dyda, dxao, dyao;
			TRIANGLE_REAL dxod2, dyod2, dxda2, dyda2, dxao2, dyao2;
			TRIANGLE_REAL apexlen, orglen, destlen, minedge;
			TRIANGLE_REAL angle;
			TRIANGLE_REAL area;
			TRIANGLE_REAL dist1, dist2;
			subseg sptr;                      /* Temporary variable used by tspivot(). */
...

		{
			...
			TRIANGLE_REAL orgorient, destorient;
			int moveleft;
			triangle ptr;                         /* Temporary variable used by sym(). */
			subseg sptr;                      /* Temporary variable used by tspivot(). */
...
#ifdef ANSI_DECLARATORS
		TRIANGLE_REAL circletop(struct mesh *m, vertex pa, vertex pb, vertex pc, TRIANGLE_REAL ccwabc)
#else /* not ANSI_DECLARATORS */
		TRIANGLE_REAL circletop(m, pa, pb, pc, ccwabc)
		struct mesh *m;
		vertex pa;
		vertex pb;
		vertex pc;
		TRIANGLE_REAL ccwabc;
#endif /* not ANSI_DECLARATORS */

		{
			TRIANGLE_REAL xac, yac, xbc, ybc, xab, yab;
			TRIANGLE_REAL aclen2, bclen2, ablen2;
		...
		struct splaynode *splayroot,
		struct otri *newkey,
			vertex pa, vertex pb, vertex pc, TRIANGLE_REAL topy)
#else /* not ANSI_DECLARATORS */
		...
		{
			TRIANGLE_REAL ccwabc;
			TRIANGLE_REAL xac, yac, xbc, ybc;
			TRIANGLE_REAL aclen2, bclen2;
			TRIANGLE_REAL searchpoint[2];
...

		{
			...
			TRIANGLE_REAL lefttest, righttest;
			int heapsize;
		...
		int reconstruct(struct mesh *m, struct behavior *b, int *trianglelist,
			TRIANGLE_REAL *triangleattriblist, TRIANGLE_REAL *trianglearealist,
		...
		struct behavior *b;
		int *trianglelist;
		TRIANGLE_REAL *triangleattriblist;
		TRIANGLE_REAL *trianglearealist;
		int elements;
		int corners;
		int attribs;
		int *segmentlist;
			...
			while (triangleloop.tri != (triangle *) NULL) {
				...
				for (j = 0; j < m->eextras; j++) {
					...
					if (*stringptr == '\0') {
						...
						setelemattribute(triangleloop, j,
							(TRIANGLE_REAL) strtod(stringptr, &stringptr));
					}
#endif /* not TRILIBRARY */
				}
				...
				if (b->vararea) {
					...
					if (*stringptr == '\0') {
						...
						area = (TRIANGLE_REAL) strtod(stringptr, &stringptr);
					}
#endif /* not TRILIBRARY */
...

		{
			...
			vertex startvertex;
			vertex leftvertex, rightvertex;
			TRIANGLE_REAL leftccw, rightccw;
...

		{
			...
			TRIANGLE_REAL ex, ey;
			TRIANGLE_REAL tx, ty;
			TRIANGLE_REAL etx, ety;
			TRIANGLE_REAL split, denom;
			int i;
			triangle ptr;                       /* Temporary variable used by onext(). */
		...
		void regionplague(struct mesh *m, struct behavior *b,
			TRIANGLE_REAL attribute, TRIANGLE_REAL area)
#else /* not ANSI_DECLARATORS */
		void regionplague(m, b, attribute, area)
		struct mesh *m;
		struct behavior *b;
		TRIANGLE_REAL attribute;
		TRIANGLE_REAL area;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
		void carveholes(struct mesh *m, struct behavior *b, TRIANGLE_REAL *holelist, int holes,
			TRIANGLE_REAL *regionlist, int regions)
#else /* not ANSI_DECLARATORS */
		void carveholes(m, b, holelist, holes, regionlist, regions)
		struct mesh *m;
		struct behavior *b;
		TRIANGLE_REAL *holelist;
		int holes;
		TRIANGLE_REAL *regionlist;
		int regions;
#endif /* not ANSI_DECLARATORS */
...

		{
			...
			vertex newvertex;
			enum insertvertexresult success;
			TRIANGLE_REAL segmentlength, nearestpoweroftwo;
			TRIANGLE_REAL split;
			TRIANGLE_REAL multiplier, divisor;
...

		{
			...
			for (i = 0; i < m->invertices; i++) {
				...
				x = (TRIANGLE_REAL) strtod(stringptr, &stringptr);
				stringptr = findfield(stringptr);
				if (*stringptr == '\0') {
					//printf("Error:  Vertex %d has no y coordinate.\n", b->firstnumber + i);
					triexit(1);
				}
				y = (TRIANGLE_REAL) strtod(stringptr, &stringptr);
				vertexloop[0] = x;
				vertexloop[1] = y;
				...
				for (j = 2; j < 2 + m->nextras; j++) {
					...
					if (*stringptr == '\0') {
						...
						vertexloop[j] = (TRIANGLE_REAL) strtod(stringptr, &stringptr);
					}
				}
				if (nodemarkers) {
...
#ifdef ANSI_DECLARATORS
		void transfernodes(struct mesh *m, struct behavior *b, TRIANGLE_REAL *pointlist,
			TRIANGLE_REAL *pointattriblist, int *pointmarkerlist,
		...
		struct mesh *m;
		struct behavior *b;
		TRIANGLE_REAL *pointlist;
		TRIANGLE_REAL *pointattriblist;
		int *pointmarkerlist;
		int numberofpoints;
		...
		void readholes(struct mesh *m, struct behavior *b,
			FILE *polyfile, char *polyfilename, TRIANGLE_REAL **hlist, int *holes,
			TRIANGLE_REAL **rlist, int *regions)
#else /* not ANSI_DECLARATORS */
...

		{
			...
			*holes = (int) strtol(stringptr, &stringptr, 0);
			if (*holes > 0) {
				holelist = (TRIANGLE_REAL *) trimalloc(2 * *holes * (int) sizeof(TRIANGLE_REAL));
				*hlist = holelist;
				for (i = 0; i < 2 * *holes; i += 2) {
					stringptr = readline(inputline, polyfile, polyfilename);
					stringptr = findfield(stringptr);
					if (*stringptr == '\0') {
						...
						holelist[i] = (TRIANGLE_REAL) strtod(stringptr, &stringptr);
					}
					stringptr = findfield(stringptr);
					if (*stringptr == '\0') {
						...
						holelist[i + 1] = (TRIANGLE_REAL) strtod(stringptr, &stringptr);
					}
				}
			} else {
				*hlist = (TRIANGLE_REAL *) NULL;
			}
			...
			if ((b->regionattrib || b->vararea) && !b->refine) {
				...
				*regions = (int) strtol(stringptr, &stringptr, 0);
				if (*regions > 0) {
					regionlist = (TRIANGLE_REAL *) trimalloc(4 * *regions * (int) sizeof(TRIANGLE_REAL));
					*rlist = regionlist;
					index = 0;
					for (i = 0; i < *regions; i++) {
						stringptr = readline(inputline, polyfile, polyfilename);
						...
						if (*stringptr == '\0') {
							...
							regionlist[index++] = (TRIANGLE_REAL) strtod(stringptr, &stringptr);
						}
						stringptr = findfield(stringptr);
						if (*stringptr == '\0') {
...
#ifdef ANSI_DECLARATORS
		void writenodes(struct mesh *m, struct behavior *b, TRIANGLE_REAL **pointlist,
			TRIANGLE_REAL **pointattriblist, int **pointmarkerlist)
#else /* not ANSI_DECLARATORS */
...

		{
			...
			if (*pointlist == (TRIANGLE_REAL *) NULL) {
				*pointlist = (TRIANGLE_REAL *) trimalloc((int) (outvertices * 2 * sizeof(TRIANGLE_REAL)));
			}
			/* Allocate memory for output vertex attributes if necessary. */
			if ((m->nextras > 0) && (*pointattriblist == (TRIANGLE_REAL *) NULL)) {
				*pointattriblist = (TRIANGLE_REAL *) trimalloc((int) (outvertices * m->nextras *
					sizeof(TRIANGLE_REAL)));
			}
		...
		void writeelements(struct mesh *m, struct behavior *b,
			int **trianglelist, TRIANGLE_REAL **triangleattriblist)
#else /* not ANSI_DECLARATORS */
...

		{
			...
			if ((m->eextras > 0) && (*triangleattriblist == (TRIANGLE_REAL *) NULL)) {
				*triangleattriblist = (TRIANGLE_REAL *) trimalloc((int) (m->triangles.items *
					m->eextras *
					sizeof(TRIANGLE_REAL)));
			}
			tlist = *trianglelist;
		...
		void writepoly(struct mesh *m, struct behavior *b, char *polyfilename,
			TRIANGLE_REAL *holelist, int holes, TRIANGLE_REAL *regionlist, int regions,
			int argc, char **argv)
#else /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
		void writevoronoi(struct mesh *m, struct behavior *b, TRIANGLE_REAL **vpointlist,
			TRIANGLE_REAL **vpointattriblist, int **vpointmarkerlist,
			int **vedgelist, int **vedgemarkerlist, TRIANGLE_REAL **vnormlist)
#else /* not ANSI_DECLARATORS */
		void writevoronoi(m, b, vpointlist, vpointattriblist, vpointmarkerlist,
			vedgelist, vedgemarkerlist, vnormlist)
		struct mesh *m;
		struct behavior *b;
		TRIANGLE_REAL **vpointlist;
		TRIANGLE_REAL **vpointattriblist;
		int **vpointmarkerlist;
		int **vedgelist;
		int **vedgemarkerlist;
		TRIANGLE_REAL **vnormlist;
#endif /* not ANSI_DECLARATORS */
...

		{
			...
			if (*vpointlist == (TRIANGLE_REAL *) NULL) {
				*vpointlist = (TRIANGLE_REAL *) trimalloc((int) (m->triangles.items * 2 *
					sizeof(TRIANGLE_REAL)));
			}
			/* Allocate memory for Voronoi vertex attributes if necessary. */
			if (*vpointattriblist == (TRIANGLE_REAL *) NULL) {
				*vpointattriblist = (TRIANGLE_REAL *) trimalloc((int) (m->triangles.items *
					m->nextras * sizeof(TRIANGLE_REAL)));
			}
			...
			*vedgemarkerlist = (int *) NULL;
			...
			if (*vnormlist == (TRIANGLE_REAL *) NULL) {
				*vnormlist = (TRIANGLE_REAL *) trimalloc((int) (m->edges * 2 * sizeof(TRIANGLE_REAL)));
			}
			elist = *vedgelist;
			normlist = *vnormlist;
			coordindex = 0;
...

		{
			struct otri triangleloop;
			vertex p[3];
			TRIANGLE_REAL cossquaretable[8];
			TRIANGLE_REAL ratiotable[16];
			TRIANGLE_REAL dx[3], dy[3];
			TRIANGLE_REAL edgelength[3];
			TRIANGLE_REAL dotproduct;
			TRIANGLE_REAL cossquare;
			TRIANGLE_REAL triarea;
			TRIANGLE_REAL shortest, longest;
			TRIANGLE_REAL trilongest2;
			TRIANGLE_REAL smallestarea, biggestarea;
			TRIANGLE_REAL triminaltitude2;
			TRIANGLE_REAL minaltitude;
			TRIANGLE_REAL triaspect2;
			TRIANGLE_REAL worstaspect;
			TRIANGLE_REAL smallestangle, biggestangle;
			TRIANGLE_REAL radconst, degconst;
			int angletable[18];
			int aspecttable[16];
			int aspectindex;
			int tendegree;
			...
			degconst = 180.0 / PI;
			for (i = 0; i < 8; i++) {
				cossquaretable[i] = cos(radconst * (TRIANGLE_REAL) (i + 1));
...

		{
			struct mesh m;
			struct behavior b;
			TRIANGLE_REAL *holearray;                                        /* Array of holes. */
			TRIANGLE_REAL *regionarray;   /* Array of regional attributes and area constraints. */
#ifndef TRILIBRARY
			FILE *polyfile;
#endif /* not TRILIBRARY */
#ifndef NO_TIMER
			...
			if (b.poly || b.convex) {
				...
				if (b.nopolywritten || b.noiterationnum) {
					...
					if (b.poly) {
						...
						out->holelist = (TRIANGLE_REAL *) NULL;
						out->regionlist = (TRIANGLE_REAL *) NULL;
					}
#else /* not TRILIBRARY */

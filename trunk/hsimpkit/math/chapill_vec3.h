/****************************************************************************
IT IS SUCH A PITY THAT I HAVE TO CHANGE THE VECTORS' NAME, DUE TO THAT THE 
'vec3' HAS BEEN DEFINED IN 'libgfx' AS A CLASS (NOT TEMPLATE), LIKE WHAT IS IN
THE FILE 'libgfx\include\gfx\vec3.h'
	'typedef TVec3<double> Vec3;'
SO I'LL HAVE TO CHANGE THE TEMPLAT CLASS NAME AS WELL AS THE FILE NAME TO
'ChapillVec3', THIT IS ALSO THE CASE FOR 'Vec4'
...
class ChapillVec3
{
  public:
    ...
    ChapillVec3 (const Type X, const Type Y, const Type Z) 
      { x=X; y=Y; z=Z; };
    /// Copy constructor
    ChapillVec3 (const ChapillVec3& v) 
      { x=v.x; y=v.y; z=v.z; };
    /// Construct from array
    ChapillVec3 (const Type v[3])
      { x=v[0]; y=v[1]; z=v[2]; };
    /// Set from components
    ...
    bool operator == (const ChapillVec3& A) const       /// COMPARISON (==)
      { return (x==A.x && y==A.y && z==A.z); }
    bool operator != (const ChapillVec3& A) const       /// COMPARISON (!=)
      { return (x!=A.x || y!=A.y || z!=A.z); }

    bool operator > (const ChapillVec3& A) const        /// ALL greater than
      { return (x>A.x && y>A.y && z>A.z); }
    bool operator < (const ChapillVec3& A) const        /// ALL less than
      { return ((x<A.x) && (y<A.y) && (z<A.z)); }

    ChapillVec3& operator = (const ChapillVec3& A)            /// ASSIGNMENT (=)
      { x=A.x; y=A.y; z=A.z; 
        return(*this);  };
    ChapillVec3 operator + (const ChapillVec3& A) const       /// ADDITION (+)
      { ChapillVec3 Sum(x+A.x, y+A.y, z+A.z); 
        return(Sum); };
    ChapillVec3 operator - (const ChapillVec3& A) const       /// SUBTRACTION (-)
      { ChapillVec3 Diff(x-A.x, y-A.y, z-A.z);
        return(Diff); };
    Type operator * (const ChapillVec3& A) const       /// DOT-PRODUCT (*)
      { Type DotProd = x*A.x+y*A.y+z*A.z; 
        return(DotProd); };
    ChapillVec3 operator / (const ChapillVec3& A) const       /// CROSS-PRODUCT (/)
      { ChapillVec3 CrossProd(y*A.z-z*A.y, z*A.x-x*A.z, x*A.y-y*A.x);
        return(CrossProd); };
    ChapillVec3 operator ^ (const ChapillVec3& A) const       /// ALSO CROSS-PRODUCT (^)
      { ChapillVec3 CrossProd(y*A.z-z*A.y, z*A.x-x*A.z, x*A.y-y*A.x);
        return(CrossProd); };
    ChapillVec3 operator * (const Type s) const        /// MULTIPLY BY SCALAR V*s (*)
      { ChapillVec3 Scaled(x*s, y*s, z*s); 
        return(Scaled); };
    ChapillVec3 operator / (const Type s) const        /// DIVIDE BY SCALAR (/)
      { ChapillVec3 Scaled(x/s, y/s, z/s);
        return(Scaled); };
    ChapillVec3 operator & (const ChapillVec3& A) const       /// COMPONENT MULTIPLY (&)
      { ChapillVec3 CompMult(x*A.x, y*A.y, z*A.z);
        return(CompMult); }

    friend inline ChapillVec3 operator *(Type s, const ChapillVec3& v)  /// SCALAR MULT s*V
      { return ChapillVec3(v.x*s, v.y*s, v.z*s); }

    ChapillVec3& operator += (const ChapillVec3& A)    /// ACCUMULATED VECTOR ADDITION (+=)
      { x+=A.x; y+=A.y; z+=A.z;
        return *this;}
    ChapillVec3& operator -= (const ChapillVec3& A)    /// ACCUMULATED VECTOR SUBTRACTION (-=)
      { x-=A.x; y-=A.y; z-=A.z; 
        return *this; }
    ChapillVec3& operator *= (const Type s)     /// ACCUMULATED SCALAR MULT (*=)
      { x*=s; y*=s; z*=s; 
        return *this; }
    ChapillVec3& operator /= (const Type s)     /// ACCUMULATED SCALAR DIV (/=)
      { x/=s; y/=s; z/=s; 
        return *this; }
    ChapillVec3& operator &= (const ChapillVec3& A)  /// ACCUMULATED COMPONENT MULTIPLY (&=)
      { x*=A.x; y*=A.y; z*=A.z; return *this; }
    ChapillVec3 operator - (void) const        /// NEGATION (-)
      { ChapillVec3 Negated(-x, -y, -z);
        return(Negated); };
...
*/
    ...
    Type LengthSqr (void) const                  /// LENGTH OF VECTOR (SQUARED)
      { return (x*x+y*y+z*z); };
    ChapillVec3& Normalize (void)                       /// NORMALIZE VECTOR
     ...
     *  bounding boxes.
     */
    void UpdateMinMax(ChapillVec3 &Min, ChapillVec3 &Max) const
    {
     ...
     */
  void CompleteOrthonormalBasis(ChapillVec3 &U, ChapillVec3 &V) const
    {
      U = *this;
    ...
    /// This is a handy way to get the zero vector just use ChapillVec3<Type>::ZERO
    static ChapillVec3 ZERO;
};

//typedef Vec3<float> Vec3f;
//typedef Vec3<double> Vec3d;
//typedef Vec3<double> CVertex; // chapel hill vertex
//typedef Vec3<int> VertexPos;

template<class Type> ChapillVec3<Type> ChapillVec3<Type>::ZERO = ChapillVec3<Type>(0,0,0);

/*
	modified by ht for convert of ascii ply file
	to binary ply file
*/
...
int convert_ply(/*int argc, char *argv[], */const char* infilename, const char* outfilename, int verbose_flag)
{
  int i,j,k;
  PlyFile *in_ply;
  PlyFile *out_ply;
  ...
  if(fin == NULL)
  {
	cout << "input file open failes" << endl;
	return 0;
  }
  FILE *fout = fopen(outfilename, "w");
  if(fout == NULL)
  {
	  cout << "output file open failes" << endl;
	  return 0;
  }
...
#ifdef WRITE_BINARY
  out_ply = write_ply (fout, nelems, elist, PLY_BINARY_BE);
#endif
